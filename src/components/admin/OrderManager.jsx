...
import { Truck, CheckCircle, XCircle, Clock, Search, Eye, X, Loader2, ArrowRight } from 'lucide-react';
...
export const OrderManager = ({ notify }) => {
  ...
  useEffect(() => {
    fetchOrders();
  }, [searchQuery]); // Re-fetch when search changes (debounce ideally)

  const fetchOrders = async () => {
    setLoading(true);
    try {
      if (searchQuery.trim().length > 5) {
         // Direct ID Search attempt
  ...
  return (
    <div className="space-y-6">
                          ...
                          <div key={i} className="flex gap-4 p-3 border border-gray-100 rounded-lg bg-gray-50/50">
                             ...
                             <div className="flex-1">
                                ...
                                <div className="mt-2 flex gap-2">
                                   {item.options?.fallPico && <span className="text-[10px] bg-white border px-1 rounded">Fall/Pico</span>}
                                </div>
                             </div>
      ...
      {/* Re-use Shipping Modal Logic from previous code but keep it minimal (omitted for brevity, assume same logic as before just styled) */}
      <AnimatePresence>
        {shippingModal.open && (
