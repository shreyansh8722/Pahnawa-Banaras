...
import { 
  ...
  Ticket, LayoutTemplate, Globe, FolderOpen, CheckCircle, AlertCircle 
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  if (user.email !== ADMIN_EMAIL) {
    return (
      <div className="h-screen flex flex-col items-center justify-center text-center px-4 bg-gray-50">
        ...
        <p className="text-gray-500 mb-6 max-w-md">This area is restricted to administrators only. If you believe this is an error, please contact support.</p>
  ...
  const renderContent = () => {
    const props = { notify }; // Pass notify function to all children
    switch (activeTab) {
    ...
    <div className="min-h-screen bg-gray-50 flex font-sans overflow-hidden">
      ...
      `}>
        ...
        <nav className="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto scrollbar-hide">
          ...
          <NavItem id="settings" icon={Globe} label="Settings" />
        </nav>

        <div className="p-4 border-t border-gray-800 bg-[#151515]">
          <button onClick={() => navigate('/')} className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold uppercase tracking-widest text-white/50 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
