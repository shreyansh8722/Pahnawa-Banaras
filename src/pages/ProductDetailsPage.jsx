...
import { ChevronRight, Heart, Check, Zap, ShieldCheck, Truck, RotateCcw, ShoppingBag } from 'lucide-react';
...
export default function ProductDetailsPage() {
  ...
  if (!product) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white p-4 font-sans">
      ...
      <button onClick={() => navigate('/shop')} className="bg-[#B08D55] text-white px-6 py-3 uppercase text-xs font-bold tracking-widest rounded-sm">Back to Shop</button>
    </div>
  );
          ...
          <div className="md:sticky md:top-28">
            <div className="mb-6 mt-4 md:mt-0">
              <div className="flex justify-between items-start mb-2">
                  <h1 className="font-serif text-2xl md:text-4xl text-gray-900 leading-tight max-w-md">{product.name}</h1>
                  <button onClick={() => toggleFavorite(product.id)} className="p-2 rounded-full bg-gray-50 md:bg-transparent hover:bg-gray-100 transition-colors">
                      <Heart size={24} fill={isFavorite ? "#ef4444" : "none"} className={isFavorite ? "text-red-500" : "text-gray-500"} />
                  </button>
              </div>
              ...
              <div className="flex items-baseline gap-3 mb-4">
                <span className="text-2xl md:text-3xl font-bold text-gray-900 font-serif">₹{finalPrice.toLocaleString('en-IN')}</span>
                {product.comparePrice > product.price && <span className="text-sm text-gray-400 line-through">MRP ₹{product.comparePrice?.toLocaleString('en-IN')}</span>}
              </div>
            </div>
            ...
            <DeliveryChecker />
            <ProductTabs description={product.fullDescription} material={product.material} weave={product.subCategory} sku={product.id.slice(0,8).toUpperCase()} />
            ...
            <div className="grid grid-cols-3 gap-4 py-8 border-t border-gray-100 mt-8">
              ...
              <div className="text-center"><RotateCcw size={20} className="mx-auto text-[#B08D55] mb-2"/><p className="text-[10px] uppercase font-bold tracking-wider text-gray-500">7 Day Returns</p></div>
            </div>
          </div>
        </div>
      </div>
