...
export default function ProductDetailsPage() {
  ...
  const handleAddToCart = () => {
    ...
    addToCart(cartItem);
    toast.success("Added to Bag"); // Use global toaster
  };
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-brand-dark flex flex-col relative">
      <SEO title={product.name} description={product.fullDescription?.slice(0, 150)} image={product.featuredImageUrl} />
      <Navbar />

      <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16 mb-10 px-0 md:px-8 pt-0 pb-12">
        <div className="md:col-span-7 w-full"><ProductGallery images={images} name={product.name} /></div>
        ...
        <div className="md:col-span-5 px-4 md:px-8">
          <div className="md:sticky md:top-28">
            <div className="mb-6 mt-4 md:mt-0">
              <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button onClick={handleShare} className="p-2 rounded-full bg-gray-50 hover:bg-gray-100 transition-colors text-gray-600"><Share2 size={20} /></button>
                    <button onClick={() => toggleFavorite(product.id)} className="p-2 rounded-full bg-gray-50 hover:bg-gray-100 transition-colors"><Heart size={20} fill={isFavorite ? "#ef4444" : "none"} className={isFavorite ? "text-red-500" : "text-gray-600"} /></button>
                  </div>
              </div>
              
              <div className="flex flex-wrap gap-2 mb-4">
                <span className="text-[10px] text-gray-500 uppercase tracking-wider bg-gray-100 px-2 py-1 rounded-sm font-bold">{product.subCategory || 'Banarasi'}</span>
                {isOutOfStock ? <span className="flex items-center gap-1 text-[10px] font-bold text-white bg-gray-900 px-2 py-1 rounded-sm"><XCircle size={12} /> SOLD OUT</span> : <span className="flex items-center gap-1 text-[10px] font-bold text-green-700 bg-green-50 px-2 py-1 rounded-sm"><Check size={12} /> In Stock</span>}
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="p-3 bg-gray-50 rounded-sm border border-gray-100"><p className="text-[10px] uppercase text-gray-400 font-bold tracking-widest mb-1">Blouse Piece</p><p className="text-xs font-medium text-gray-900 flex items-center gap-1"><Check size={12} className="text-green-600"/> Included</p></div>
                <button onClick={() => setShowSizeChart(true)} className="p-3 bg-white rounded-sm border border-gray-200 hover:border-[#B08D55] transition-colors text-left group"><p className="text-[10px] uppercase text-gray-400 font-bold tracking-widest mb-1 group-hover:text-[#B08D55]">Size Guide</p><p className="text-xs font-medium text-gray-900 flex items-center gap-1"><Ruler size={12} /> View Chart</p></button>
            </div>

            <ProductCustomization options={customizations} onOptionsChange={setCustomizations} />
            
            {/* MAIN BUTTONS - Ref attached here */}
            <div className="flex gap-3 mb-8 mt-6 items-stretch" ref={mainButtonRef}>
              <button onClick={handleAddToCart} disabled={isOutOfStock} className="flex-1 bg-white border border-black text-black py-3.5 md:py-4 text-xs font-bold uppercase tracking-widest hover:bg-gray-50 transition-all shadow-sm flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"><ShoppingBag size={16} /> {isOutOfStock ? "Sold Out" : "Add to Bag"}</button>
              <button onClick={handleBuyNow} disabled={isOutOfStock} className="flex-1 bg-[#B08D55] text-white py-3.5 md:py-4 text-xs font-bold uppercase tracking-widest hover:bg-[#8c6a40] transition-all shadow-lg flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"><Zap size={16} fill="currentColor"/> {isOutOfStock ? "Unavailable" : "Buy Now"}</button>
            </div>

            <DeliveryChecker />
            <ProductTabs description={product.fullDescription} material={product.material} weave={product.subCategory} sku={product.id.slice(0,8).toUpperCase()} productId={product.id} />
      ...
      {/* Sticky Bar only appears when main buttons are hidden */}
      <StickyActionBar 
        product={product} 
